...
class FormManager {
        ...
        return new Promise((resolve) => {
            ...
            request.onsuccess = () => {
                if (request.result) {
                    ...
                    document.getElementById('schema-input').value = JSON.stringify(schema, null, 4);
    ...
    setupEventListeners() {
        ...
        // Form generation
        document.getElementById('generate-form-btn').addEventListener('click', () => {
        ...
        document.getElementById('load-example-btn').addEventListener('click', () => {
            fetch('template/form.json')
                .then(r => r.json())
                .then(data => {
                    document.getElementById('schema-input').value = JSON.stringify(data, null, 4);
                })
                .catch(() => {
                    ...
                    const example = {
                        "formTitle": "示例表单",
                        "fields": [
                            { "label": "姓名", "fieldName": "name", "type": "input", "required": true },
                            { "label": "意见", "fieldName": "feedback", "type": "textarea" }
                        ]
                    };
                    document.getElementById('schema-input').value = JSON.stringify(example, null, 4);
                });
        });
    }
    ...
    renderDataTable(submissions) {
        ...
        if (submissions.length === 0) {
            ...
            body.innerHTML = '<tr><td style="text-align:center">请先填写表单并提交</td></tr>';
            return;
        }
        ...
        head.innerHTML = columns.map(k => `<th>${labelMap[k] || k}</th>`).join('');
        ...
        body.innerHTML = submissions.map(s => {
            return `<tr>
                ...
                <td>${s.schemaTitle}</td>
                ${columns.slice(3).map(k => `<td>${s.data[k] || '-'}</td>`).join('')}
            </tr>`;
        }).join('');
    }
    ...
    exportToCSV() {
        ...
        request.onsuccess = () => {
            ...
            data.forEach(s => Object.keys(s.data).forEach(k => keys.add(k)));
            const columns = Array.from(keys);
    ...
    showToast(msg, type = 'success') {
        ...
        toast.style.background = type === 'error' ? 'var(--danger-color)' : '#333';
